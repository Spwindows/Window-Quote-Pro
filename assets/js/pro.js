...
function renderProUI() {
  ...
  const accountPanel = el('pro-account-panel') || el('pro-account-card') || dashboard;
  const teamSetup = el('pro-team-setup');
  const teamDash = el('pro-team-dashboard');
  const headerBadge = el('header-badge') || el('pro-badge');

  if (!proState.user) {
    if (authPanel) authPanel.classList.remove('hidden');
    if (dashboard) dashboard.classList.add('hidden');
    if (accountPanel && accountPanel !== dashboard) accountPanel.classList.add('hidden');
  ...
  if (accountPanel && accountPanel !== dashboard) accountPanel.classList.remove('hidden');

  const displayName = proState.user.user_metadata?.full_name || proState.user.email || 'My Account';
  const nameEl = el('pro-user-name') || el('team-name-display');
  const emailEl = el('pro-user-email') || el('team-email-display');
  const roleEl = el('team-role-badge');

  if (nameEl) {
    nameEl.textContent = proState.teamName || displayName;
  }
  if (emailEl) {
    emailEl.textContent = proState.user.email || '';
  }
  if (roleEl) {
    const role = (proState.teamRole || (proState.teamId ? 'staff' : 'owner')).toLowerCase();
    roleEl.textContent = role.charAt(0).toUpperCase() + role.slice(1);
    roleEl.className = `role-badge ${role === 'owner' ? 'role-owner' : 'role-staff'}`;
  }
  ...
  const logoSection = el('logo-section') || el('logo-upload-section');
  if (logoSection) {
