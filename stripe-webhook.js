...
function planFromObject(obj) {
  if (!obj) return 'free';
  const metaPlan = obj.metadata && obj.metadata.plan;
  if (metaPlan === 'pro_solo' || metaPlan === 'pro_team') return metaPlan;

  if (obj.items && obj.items.data && obj.items.data.length > 0) {
...
async function handleCheckoutCompleted(session) {
  ...
  const plan = planFromObject(session);
...
async function handleSubscriptionCreatedOrUpdated(subscription) {
  ...
  const plan = planFromObject(subscription);
